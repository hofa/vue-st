var PE = {
    ...
    mileageListCB: function(res) {
        ...
        if (typeof res.data != "undefined" && res.data.length > 0) {
            for(var d in res.data) {
                ...
                res.data[d].acce = res.data[d].acce < 10 ? '0' + res.data[d].acce : res.data[d].acce;
                res.data[d].dece = res.data[d].dece < 10 ? '0' + res.data[d].dece : res.data[d].dece;
                //var tabTitle = CM.formatDate(res.data[d].collect_date, "yyyy年MM月");
                var tabTitle = new Date(res.data[d].collect_date.replace(/-/g, "/")).Format("yyyy年MM月");
    ...
    init: function () {
        ...
        //     },
        // });
        PE.size = parseInt($(window).height() / 46);
        //console.log(PE.size);
        $("#carhistory").scroll(function() {
            if (PE.dyc == true) PE.mileageList();
        });
    }
}
